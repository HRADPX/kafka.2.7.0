...
public class NetworkReceive implements Receive {
    ...
    public boolean complete() {
        // buffer != null 表示有足够的内存大小分配
        // !buffer.hasRemaining() 表示消息已经完整读取
        return !size.hasRemaining() && buffer != null && !buffer.hasRemaining();
    }

    /**
     * 读取服务端响应，它处理粘包和拆包的问题
     * 处理方式: 响应消息由两部分组成（int 类型的数 + 响应消息），int 的值就是消息的大小..
     */
    public long readFrom(ScatteringByteChannel channel) throws IOException {
        ...
        if (size.hasRemaining()) {
            ...
            read += bytesRead;
            // 如果还有剩余空间，表示有拆包，不会分配内存空间，会直接返回，会在下次进入这个方法再次读取，
            // 直到这个 size 读满才表示，它后面消息的实际大小
            if (!size.hasRemaining()) {
                size.rewind();
                // 消息的实际大小
