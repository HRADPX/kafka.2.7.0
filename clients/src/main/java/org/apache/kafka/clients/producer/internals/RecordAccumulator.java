...
public final class RecordAccumulator {
    ...
    public RecordAccumulator(LogContext logContext,
        ...
        //  CopyOnWriteMap，这是 Kafka 设计的数据结构
    ...
    public RecordAppendResult append(TopicPartition tp,
        ...
        try {
            ...
            synchronized (dq) {
                ...
                // 尝试将消息追加消息
                // 1) 有末尾批记录，返回 null，后面创建批记录，所以首次添加一定会失败的.
                // 2) 有末尾批记录，但是批记录已满，返回 null.
                // 3) 添加成功，返回
            ...
            /*
             * 计算批次的大小
             *   在消息大小和批次大小之间取一个最大值，用这个值作为批记录的大小。因为有可能一个消息的大小比默认的批次记录还要大。
             *   默认批记录大小为 16K
             *   如果生产者发送的消息超过批记录大小，这就意味着一个消息就是一个批次，即消息是一条一条被发送出去的，这样批次的大小就没有意义了。
             */
            ...
            synchronized (dq) {
                ...
                if (appendResult != null) {
                    ...
                    // finally will deallocate this thread's buffer
                    return appendResult;
                }
        ...
        for (Map.Entry<TopicPartition, Deque<ProducerBatch>> entry : this.batches.entrySet()) {
            ...
            synchronized (deque) {
                while (!deque.isEmpty()) {
                    ...
                    // 判断是否超时
                    // deliveryTimeoutMs 请求发送的超时时间，默认是 120s
     ...
     * </ol>
     * 数据 ready 的条件：
     * 1) 第一个批记录已满（队列长度大于1，末尾批记录已满）
     * 2) 发送线程关闭，需要立刻当前队列中的消息
     * 3) 消息等待超过最大的等待时间
     * 4) 内存池不足
     */
    public ReadyCheckResult ready(Cluster cluster, long nowMs) {
        ...
        // 表示内存池空间不够了，有批记录没有申请到足够内存空间，阻塞等待其他线程归还内存
        // exhausted = true 表示有线程在等待内存，需要立即发送批记录来释放空间...
        ...
        for (Map.Entry<TopicPartition, Deque<ProducerBatch>> entry : this.batches.entrySet()) {
            ...
            synchronized (deque) {
                ...
                if (batch != null) {
                    ...
                    // 当前分区的主副本节点还没找到，这时后续会更新元数据
                    if (leader == null) {
                        // This is a partition for which leader is not known, but messages are available to send.
                        // Note that entries are currently not removed from batches when deque is empty.
                        unknownLeaderTopics.add(part.topic());
                    } else if (!readyNodes.contains(leader) && !isMuted(part)) {
                        // 当前时间 - 上次重试的时间 = 已经等待的时间
                        long waitedTimeMs = batch.waitedTimeMs(nowMs);
                        /**
                         * batch.attempts() > 0 表示重试次数，这个批次之前发送失败过...
                         * retryBackoffMs: 重试时间间隔，默认 100ms
                         * backingOff 为 true，已经等待的时间小于重试间隔，表示还没有到发送的时间...
                         * backingOff 为 false， 已经等待的时间超过了时间间隔，表示到了发送的时间...
                         */
                        boolean backingOff = batch.attempts() > 0 && waitedTimeMs < retryBackoffMs;
                        /**
                         * timeToWaitMs: 还需要等待的时间
                         * lingerMs: 表示如果可以发送，是否需要额外等待一段时间，这是为了让一次请求可以更多的发送批记录
                         * @see ProducerConfig#LINGER_MS_DOC
                         */
                        long timeToWaitMs = backingOff ? retryBackoffMs : lingerMs;
                        boolean full = deque.size() > 1 || batch.isFull();  // 第一个批记录已满
                        boolean expired = waitedTimeMs >= timeToWaitMs;     // 等待时间已到最大时间，即使没满也要发送了
    ...
    private List<ProducerBatch> drainBatchesForOneNode(Cluster cluster, Node node, int maxSize, long now) {
        ...
        do {
            ...
            synchronized (deque) {
                ...
                // 当前消息的总大小超过请求大小，结束流程
                // 在极端情况下，如果单个批记录的大小超过了请求的最大大小，那么会将这个批记录在这次或以后的某一次单个发送出去
    ...
    public Map<Integer, List<ProducerBatch>> drain(Cluster cluster, Set<Node> nodes, int maxSize, long now) {
        ...
        for (Node node : nodes) {
            // 这里是获取批次队列的第一个批记录，因为在 ready 的 node 中只能保证第一个批记录一定是满的
     ...
     * 所有的批记录都存储在 batches 这个数据结构中，它是一个 Map
     * key: 是 topicPartition
     * value: Deque
     */
