...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    KafkaProducer(Map<String, Object> configs,
        ...
        try {
            ...
            // MAX_REQUEST_SIZE_CONFIG：生产者向服务端发送消息的最大大小，默认是 1M，超过这个大小就不能发送
            this.maxRequestSize = config.getInt(ProducerConfig.MAX_REQUEST_SIZE_CONFIG);
            // BUFFER_MEMORY_CONFIG：消息缓存大小，默认是 32M
            this.totalMemorySize = config.getLong(ProducerConfig.BUFFER_MEMORY_CONFIG);
            // Kafka 支持压缩数据的格式，提高系统的吞吐量
            ...
            if (metadata != null) {
                ...
                // 元数据，生产者从服务端那拉取过来的 kafka 元数据，生产者想要去拉元数据，发送网络请求，重试
                // METADATA_MAX_AGE_CONFIG：生产者每隔一段时间都要去服务端拉取元数据更新一下自己的元数据
            ...
            // 初始化了一个重要的管理网络的组件，它实现了 Runnable 接口
            this.sender = newSender(logContext, kafkaClient, this.metadata);
            String ioThreadName = NETWORK_THREAD_PREFIX + " | " + clientId;
            // 创建了一个后台线程，传入了一个 sender 对象。把业务代码和关于线程的代码隔离开
    ...
    Sender newSender(LogContext logContext, KafkaClient kafkaClient, ProducerMetadata metadata) {
        ...
        /**
         * CONNECTIONS_MAX_IDLE_MS_CONFIG: 默认是 9min，一个网络链接最长空闲多久，超过这个时间，就关闭这个连接。
         * SEND_BUFFER_CONFIG: socket发送数据的缓冲区的大小，默认是 100Kb。
         * RECEIVE_BUFFER_CONFIG: socket接收数据的缓存区的大小，默认是 32Kb。
         */
        ...
        /**
         * RETRIES_CONFIG: 重试的次数
         * acks:
         *  0: producer 发送数据到 broker 后，直接返回，不管是否成功或失败。
         *  1: producer 发送数据到 broker 后，数据成功写入 leader partition 以后返回响应。
         *  -1: producer 发送数据到 broker 后，数据成功写入 leader partition 并且数据同步到所有的 follower partition 后，
         *  才返回响应，保证不丢失数据。
         */
    ...
    private static int configureDeliveryTimeout(ProducerConfig config, Logger log) {
        // 请求在消息收集器中等待的最大时间，默认 120s
        int deliveryTimeoutMs = config.getInt(ProducerConfig.DELIVERY_TIMEOUT_MS_CONFIG);
        int lingerMs = lingerMs(config);
        // 请求超时时间，请求发送到收到响应的最大时间间隔，默认30s
    ...
    private Future<RecordMetadata> doSend(ProducerRecord<K, V> record, Callback callback) {
        ...
        try {
            ...
            /**
             * step 6: 将消息放入到 accumulator 缓冲区中，然后由 accumulator 把消息封装成一个批次去发送。
             */
            RecordAccumulator.RecordAppendResult result = accumulator.append(tp, timestamp, serializedKey,
                    serializedValue, headers, interceptCallback, remainingWaitMs, true, nowMs);

            // abortForNewBatch 什么样场景为 true
            // 无末尾批记录或者末尾批记录已满
            ...
            /**
             * step 7: 唤醒 sender 线程，它才是真正发送数据的线程
             *  条件：如果当前批次的缓冲区满或者新创建一个批次
             */
    ...
    private ClusterAndWaitTime waitOnMetadata(String topic, Integer partition, long nowMs, long maxWaitMs) throws InterruptedException {
        ...
        // 根据当前的 topic 信息从这个集群的 cluster 元数据信息中查看分区信息。
        Integer partitionsCount = cluster.partitionCountForTopic(topic);
        // Return cached metadata if we have it, and if the record's partition is either undefined
        // or within the known partition range
        // 如果元数据中获取到分区信息，直接返回 cluster，表示之前已经获取数据，直接返回。
        // 第一次执行是不会执行这里的代码
        ...
        do {
            ...
            // 计算获取元数据花费了多长时间，如果花费时间超过最大等待时间，报超时异常
            ...
            // 尝试获取发送消息的 topic 的分区信息，如果这个值不为 null，说明 Sender 线程获取到了元数据
            partitionsCount = cluster.partitionCountForTopic(topic);
        } while (partitionsCount == null || (partition != null && partition >= partitionsCount));

        // 获取元数据成功，返回元数据信息和拉取数据花费的时间
        return new ClusterAndWaitTime(cluster, elapsed);
    }

    /**
     * Validate that the record size isn't too large
     * 单个消息的大小限制为 1M，整个缓存区大小限制为 32M
     */
    ...
    private int partition(ProducerRecord<K, V> record, byte[] serializedKey, byte[] serializedValue, Cluster cluster) {
        ...
        return partition != null ?
                partition :
                // 分区器，默认是 DefaultPartitioner，对 key hash 后对分区数量取模
