  ...
  def startup(): Unit = {
    try {
      ...
      if (canStartup) {
        ...
        // 注册 broker（在 zk 上的 /brokers/ids 创建临时目录）
        ...
        // 启动 Kafka Controller
        // 启动完成后，需要选举一个节点作为 controller 作为主节点用来管理整个 Kafka 集群
        ...
        // 创建 KafkaApis，Kafka 客户端请求最终都是这个类来处理的
        ...
        // 处理队列里的请求（线程池），默认 8 个线程...
        /** [[kafka.server.KafkaConfig.NumIoThreadsProp]] */
